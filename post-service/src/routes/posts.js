...
router.get('/:id', async (req, res) => {
  try {
    ...
    // detect user from Authorization header (optional) to set liked flag
    ...
    try {
      const r = await axios.get(`${AUTH_URL}/api/users/${post.authorId}`);
...
router.get('/author/:authorId', async (req,res) => {
  try {
    ...
    const postsWithMeta = posts.map(p => ({ ...p, likesCount: (p.likes||[]).length, liked: false, author: { username: p.authorId } }));
    res.json({ posts: postsWithMeta });
  } catch(e){ console.error(e); res.status(500).json({ message: 'error' }); }
});
...
// search posts by text (simple fulltext via regex)
router.get('/search', async (req,res) => {
  try {
    ...
    const postsWithMeta = posts.map(p => ({ ...p, likesCount: (p.likes||[]).length, liked: false }));
    res.json({ posts: postsWithMeta });
  } catch(e){ console.error(e); res.status(500).json({ message:'error' }); }
});
